...
export async function decryptPayload(
  serializedEncrypted: Uint8Array,
  recipientPrivKey: Uint8Array,
): Promise<Uint8Array> {
  const ephPubKey  = serializedEncrypted.slice(0, 65)
  const iv         = serializedEncrypted.slice(65, 77)
  const tag        = serializedEncrypted.slice(77, 93)
  const ciphertext = serializedEncrypted.slice(93)

  const sharedPoint  = secp256k1.getSharedSecret(recipientPrivKey, ephPubKey)
  const sharedSecret = sharedPoint.slice(1, 33)
  ...
  const decryptedBuffer = await crypto.subtle.decrypt(
    { name: 'AES-GCM', iv }, cryptoKey, combinedCiphertext,
  )
